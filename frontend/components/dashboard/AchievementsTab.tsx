...
export default function AchievementsTab() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center h-64 items-center gap-3">
        <Loader2 className="animate-spin text-electric-blue" size={28} />
        <p className="text-slate-500 text-xs font-mono">Loading achievements...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col justify-center h-64 items-center gap-4">
        <div className="glass rounded-2xl p-6 border border-red-400/20 text-center max-w-md">
          <p className="text-red-400 text-sm font-medium mb-2">Failed to load achievements</p>
      ...
      {items.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {items.map((item) => (
            <div
              key={item.id}
              className="glass rounded-2xl p-5 border border-white/5 hover:border-orange-highlight/20 transition-all"
            >
              ...
              {item.photos?.length > 0 && (
                <div className="flex gap-2 overflow-x-auto pb-1">
                  {item.photos.slice(0, 5).map((p: string, i: number) => (
                    <img key={i} src={p} alt="" className="w-14 h-14 rounded-xl object-cover flex-shrink-0 border border-white/10" />
                  ))}
              ...
              <div className="p-5 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-orange-highlight/50"
                    />
                  </div>
                  <div>
                    <label className="text-xs text-slate-500 font-mono mb-1.5 block">Emoji</label>
                    <input
                      value={form.icon}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-orange-highlight/50 resize-none"
                  />
                </div>
                <div>
                  ...
                  <label className="flex flex-col items-center justify-center gap-2 p-6 rounded-xl border-2 border-dashed border-white/10 hover:border-orange-highlight/30 hover:bg-orange-highlight/5 cursor-pointer transition-all">
                    {photoUploading
                      ? <><Loader2 size={20} className="animate-spin text-orange-highlight" /><p className="text-xs text-slate-400">Uploading...</p></>
                      : <><Plus size={20} className="text-slate-400" /><p className="text-xs text-slate-300">Click to upload photos (multiple)</p></>
                    }
                    <input
                      type="file"
                      accept="image/*"
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      {form.photos.map((p: string, i: number) => (
                        <div key={i} className="relative group">
                          <img src={p} alt="" className="w-16 h-16 rounded-xl object-cover border border-white/10" />
                          <button
                            onClick={() => removePhoto(i)}
                            className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-red-400 rounded-full flex items-center justify-center text-white text-xs opacity-0 group-hover:opacity-100 transition-opacity"
                          >×</button>
                        </div>
                      ))}
                    </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl glass border border-white/10 text-slate-400 text-sm hover:text-white"
                  >
                    Cancel
                  </button>
                </div>
